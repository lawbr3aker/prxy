...
class GASRelay(RelayBase):
    ...
    async def send(self, request: RelayRequest):
        ...
        self._log.debug(f"POST n={len(request.packets)} pids={[p.pid for p in request.packets]}")
        try:
            async with self._session.post(
                ...
                if body.get('fallback'):
                    ...
                    # Sort by seq for stream chunks; seq=0 for plain packets is stable
                    responses.sort(key=lambda p: (p.get('pid', 0), p.get('seq', 0)))
            ...
            try:
                async with self._session.get(
                    ...
                    if isinstance(data, dict):
                        data = [data]
                    if data:
                        data.sort(key=lambda p: (p.get('pid', 0), p.get('seq', 0)))
